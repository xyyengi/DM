...
launch_background() {
  ...
  log_file="${LOG_ROOT}/station24_cdsg_lite_2b_${stamp}.log"
  pid_file="${LOG_ROOT}/station24_cdsg_lite_2b_${stamp}.pid"
  status_file="${LOG_ROOT}/station24_cdsg_lite_2b_${stamp}.status"
  nohup setsid env \
    PYTHONUNBUFFERED=1 \
    CUBLAS_WORKSPACE_CONFIG=:4096:8 \
    OMP_NUM_THREADS=1 \
    STATION24_CDSG_LITE_2B_INTERNAL_WORKER=1 \
  ...
  echo "Started Station24 CDSG-lite Experiment 2B pipeline"
  echo "PID: ${pid}"
  echo "Log: ${log_file}"
...
if [[ "${STATION24_CDSG_LITE_2B_INTERNAL_WORKER:-0}" != "1" ]]; then
  launch_background
  exit 0
fi
...
CONFIG_CANDIDATE="configs/station24_state_v1_cdsg_lite_parallel_168h.yaml"
...
for key in [
    ...
    baseline_core["model"].pop(key, None)
    candidate_core["model"].pop(key, None)
if baseline_core != candidate_core:
    raise SystemExit("CDSG-lite configs differ outside approved fusion fields")
if candidate["model"].get("spatial_mix_levels") != ["bottleneck"]:
    raise SystemExit("candidate must preserve the baseline bottleneck graph")
if candidate["model"].get("parallel_spatial_fusion_levels") != ["encoder_0"]:
    raise SystemExit("candidate must use one 168h parallel fusion branch")
if candidate["model"].get("state_feature_dim") != 4:
    raise SystemExit("candidate must keep the four State V1 indicators")
...
PIPELINE_ROOT="${OUTPUT_ROOT}/cdsg_lite_2b_${JOB_STAMP}"
...
ENVIRONMENT_FILE="${LOG_FILE%.log}.environment.txt"
{
  ...
  echo "experiment_family=station24_cdsg_lite_2b"
  echo "test_used=false"
  nvidia-smi
} > "${ENVIRONMENT_FILE}"

EXPERIMENT_NAME="station24_state_v1_cdsg_lite_parallel_${JOB_STAMP}"
echo "TRAIN_START variant=state_v1_cdsg_lite_parallel parallel_level=168h"
...
CANDIDATE_RESULT="${RESULT_ROOT}/state_v1_cdsg_lite_parallel_val_n${NSAMPLES}_seed${GEN_SEED}"
echo "GENERATION_START variant=state_v1_cdsg_lite_parallel members=${NSAMPLES}"
...
"${PYTHON_BIN}" tools/compare_station24_multiscale_2a.py \
  ...
  --candidate-variant state_v1_cdsg_lite_parallel \
  --baseline-label "State V1 bottleneck graph" \
  --candidate-label "CDSG-lite parallel fusion" \
  --candidate-spatial-levels bottleneck \
  --candidate-parallel-levels encoder_0 \
  --title "Station24 CDSG-lite Experiment 2B paired validation" \
  --figure-prefix cdsg_lite_2b
...
ARCHIVE="${OUTPUT_ROOT}/station24_cdsg_lite_2b_${JOB_STAMP}.tar.gz"
tar -czf "${ARCHIVE}" -C "$(dirname "${PIPELINE_ROOT}")" "$(basename "${PIPELINE_ROOT}")"
{
  echo "finished_at=$(date --iso-8601=seconds)"
  echo "BASELINE_RESULT=${BASELINE_RESULT}"
  echo "CDSG_LITE_RUN=${CANDIDATE_RUN}"
  echo "CDSG_LITE_RESULT=${CANDIDATE_RESULT}"
  echo "COMPARISON_DIR=${COMPARISON_ROOT}"
  echo "TIMING_DIAGNOSTIC_DIR=${TIMING_ROOT}"
  echo "ARCHIVE=${ARCHIVE}"
  echo "ALL_STATION24_CDSG_LITE_2B_EXPERIMENTS_COMPLETED"
} > "${RESULT_FILE}"
cat "${RESULT_FILE}"
