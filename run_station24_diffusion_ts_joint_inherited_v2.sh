...
MODE=${1:-run}
ROOT=${2:-outputs_shandong/station24/diffusion_ts_joint_inherited_v2_bf16_$(date +%Y%m%d_%H%M%S)}
...
CONFIG=configs/station24_diffusion_ts_joint_inherited_v2_bf16.yaml
...
python -m tools.diagnose_station24_wind_event_timing "$BASELINE" "$ROOT/validation_results" --output-dir "$POST/wind_timing" --baseline-variant geo_history_actual_body_tail_moe_raw --candidate-variant station24_diffusion_ts_joint_inherited_v2_bf16 --baseline-label 'Raw body-tail' --candidate-label 'TS inherited V2 BF16'
