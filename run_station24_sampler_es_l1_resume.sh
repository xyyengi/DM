...
shopt -s nullglob
runs=("${PIPELINE_ROOT}"/training/*_station24_sampler_es_l1_*_seed2027)
shopt -u nullglob
[[ ${#runs[@]} -eq 1 ]] || {
  echo "expected one completed L1 training run, found ${#runs[@]}" >&2
  exit 1
}
RUN=${runs[0]}
...
RESULT="${PIPELINE_ROOT}/validation_results/geo_history_actual_body_tail_sampler_es_l1_val_n${NSAMPLES}_seed${GEN_SEED}"
...
if [[ ! -f "${AUDIT}/sampler_es_l1_audit.json" ]]; then
...
echo "SAMPLER_ES_L1_RESUME_COMPLETE"
