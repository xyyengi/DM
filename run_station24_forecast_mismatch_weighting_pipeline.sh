...
EXPECTED_BRANCH=${EXPECTED_BRANCH:-experiment/24site-forecast-mismatch-weighting}
...
launch_background() {
  ...
  log_file="${LOG_ROOT}/station24_forecast_mismatch_weighting_${stamp}.log"
  pid_file="${LOG_ROOT}/station24_forecast_mismatch_weighting_${stamp}.pid"
  status_file="${LOG_ROOT}/station24_forecast_mismatch_weighting_${stamp}.status"
  nohup setsid env \
    PYTHONUNBUFFERED=1 CUBLAS_WORKSPACE_CONFIG=:4096:8 OMP_NUM_THREADS=1 \
    STATION24_FORECAST_MISMATCH_INTERNAL_WORKER=1 JOB_STAMP="${stamp}" \
  ...
  echo "Started Station24 forecast-mismatch weighting pipeline"
  echo "PID: ${pid}"
  echo "Log: ${log_file}"
...
if [[ "${STATION24_FORECAST_MISMATCH_INTERNAL_WORKER:-0}" != "1" ]]; then
  launch_background
  exit 0
fi
...
CONFIG_WEIGHTED=configs/station24_geo_history_actual_forecast_mismatch_weighted_168h.yaml
[[ -f "${CONFIG_BASE}" && -f "${CONFIG_WEIGHTED}" ]] \
  || die "forecast-mismatch configuration is missing"
...
[[ -f "${SECONDARY_GRAPH}" ]] || die "reference historical graph missing: ${SECONDARY_GRAPH}"

"${PYTHON_BIN}" - "${CONFIG_BASE}" "${CONFIG_WEIGHTED}" "${DATA}" "${SECONDARY_GRAPH}" <<'PY'
import copy
import sys
import numpy as np
import torch
...
from station_dataset import fit_station_event_weighting, load_station_static_data

base = yaml.safe_load(open(sys.argv[1], encoding="utf-8"))
candidate = yaml.safe_load(open(sys.argv[2], encoding="utf-8"))
event_keys = {
    "use_extreme_event_weighting", "event_weighting_method", "event_max_weight",
    "event_level_quantiles", "event_ramp_mismatch_quantiles", "event_ramp_lags",
    "event_context_hours", "event_use_aggregate_events", "event_use_node_events",
}
...
if base_core != candidate_core:
    raise SystemExit("candidate changed fields outside experiment identity and event weighting")
if candidate["model"].get("event_weighting_method") != "forecast_mismatch_v1":
    raise SystemExit("forecast-mismatch weighting method is not enabled")
spec = fit_station_event_weighting(sys.argv[3], candidate["model"])
if spec["method"] != "train_forecast_mismatch_event_weighting_v1":
    raise SystemExit("fitted event-weighting audit has unexpected method")
...
if counts[0] != counts[1]:
    raise SystemExit(f"loss-only candidate changed parameter count: {counts}")
print(
    "FORECAST_MISMATCH_PARITY_OK "
    f"parameters={counts[0]} method={spec['method']} context={spec['context_hours']}h"
)
PY

PIPELINE_ROOT="${OUTPUT_ROOT}/forecast_mismatch_weighting_${JOB_STAMP}"
TRAIN_ROOT="${PIPELINE_ROOT}/training"
RESULT_ROOT="${PIPELINE_ROOT}/validation_results"
COMPARISON_ROOT="${PIPELINE_ROOT}/comparisons/history_vs_forecast_mismatch_weighted"
TIMING_ROOT="${PIPELINE_ROOT}/wind_event_timing/history_vs_forecast_mismatch_weighted"
ATTRIBUTION_ROOT="${PIPELINE_ROOT}/forecast_event_attribution/history_vs_forecast_mismatch_weighted"
TAIL_ROOT="${PIPELINE_ROOT}/extreme_wind_tail/history_vs_forecast_mismatch_weighted"
...
EXP_NAME="station24_forecast_mismatch_weighted_${JOB_STAMP}"
echo "TRAIN_START variant=geo_history_actual_forecast_mismatch_weighted"
"${PYTHON_BIN}" train_station24.py \
  --config "${CONFIG_WEIGHTED}" --data-path "${DATA}" \
...
shopt -u nullglob
[[ ${#matches[@]} -eq 1 ]] || die "expected one weighted run, found ${#matches[@]}"
WEIGHTED_RUN=${matches[0]}
cp "${ENVIRONMENT_FILE}" "${WEIGHTED_RUN}/logs/server_environment.txt"
WEIGHTED_RESULT="${RESULT_ROOT}/geo_history_actual_forecast_mismatch_weighted_val_n${NSAMPLES}_seed${GEN_SEED}"
echo "GENERATION_START variant=geo_history_actual_forecast_mismatch_weighted members=${NSAMPLES}"
"${PYTHON_BIN}" generate_station24.py \
  --run-dir "${WEIGHTED_RUN}" --data-path "${DATA}" \
  --output-dir "${WEIGHTED_RESULT}" --split val \
...
"${PYTHON_BIN}" tools/compare_station24_multiscale_2a.py \
  "${REFERENCE_HISTORY_RESULT}" "${WEIGHTED_RESULT}" \
  --data-path "${DATA}" --output-dir "${COMPARISON_ROOT}" \
  --baseline-variant geo_history_actual_dual \
  --candidate-variant geo_history_actual_forecast_mismatch_weighted \
  --baseline-label "Historical-spatial baseline" \
  --candidate-label "Forecast-mismatch weighted" \
  ...
  --title "Historical-spatial baseline versus forecast-mismatch weighting" \
  --figure-prefix history_vs_forecast_mismatch_weighted

echo "WIND_EVENT_TIMING_START"
"${PYTHON_BIN}" tools/diagnose_station24_wind_event_timing.py \
  "${REFERENCE_HISTORY_RESULT}" "${WEIGHTED_RESULT}" --data-path "${DATA}" \
...
"${PYTHON_BIN}" tools/plot_station24_extreme_tail.py \
  --baseline "${REFERENCE_HISTORY_RESULT}" --candidate "${WEIGHTED_RESULT}" \
  --data-path "${DATA}" --output-dir "${TAIL_ROOT}" --top-issues 5 \
  --baseline-label "Historical-spatial baseline" \
  --candidate-label "Forecast-mismatch weighted"

RESULT_FILE="${LOG_FILE%.log}.results.env"
ARCHIVE="${OUTPUT_ROOT}/station24_forecast_mismatch_weighting_${JOB_STAMP}.tar.gz"
tar -czf "${ARCHIVE}" -C "$(dirname "${PIPELINE_ROOT}")" "$(basename "${PIPELINE_ROOT}")"
{
  echo "finished_at=$(date --iso-8601=seconds)"
  echo "REFERENCE_HISTORY_RUN=${REFERENCE_HISTORY_RUN}"
  echo "REFERENCE_HISTORY_RESULT=${REFERENCE_HISTORY_RESULT}"
  echo "WEIGHTED_RUN=${WEIGHTED_RUN}"
  echo "WEIGHTED_RESULT=${WEIGHTED_RESULT}"
  ...
  echo "ALL_FORECAST_MISMATCH_WEIGHTING_EXPERIMENTS_COMPLETED"
} > "${RESULT_FILE}"
cat "${RESULT_FILE}"
