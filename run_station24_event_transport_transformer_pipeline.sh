...
EXPECTED_BRANCH=${EXPECTED_BRANCH:-experiment/24site-discrete-event-transport-tail}
...
launch_background() {
  ...
  log_file="${LOG_ROOT}/station24_event_transport_transformer_${stamp}.log"
  pid_file="${LOG_ROOT}/station24_event_transport_transformer_${stamp}.pid"
  status_file="${LOG_ROOT}/station24_event_transport_transformer_${stamp}.status"
  nohup setsid env PYTHONUNBUFFERED=1 CUBLAS_WORKSPACE_CONFIG=:4096:8 \
    OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 \
    STATION24_EVENT_TRANSPORT_WORKER=1 JOB_STAMP="${stamp}" \
    ...
    MEMBER_CHUNK="${MEMBER_CHUNK}" CONDA_ENV_NAME="${CONDA_ENV_NAME:-dm_env}" \
    bash "$0" > "${log_file}" 2>&1 < /dev/null &
  local pid=$!
  printf '%s\n' "${pid}" > "${pid_file}"
  printf 'state=running\npid=%s\nstarted_at=%s\n' \
    "${pid}" "$(date --iso-8601=seconds)" > "${status_file}"
  echo "Started event-transport Transformer experiment"
  echo "PID: ${pid}"
  echo "Log: ${log_file}"
...
if [[ "${STATION24_EVENT_TRANSPORT_WORKER:-0}" != "1" ]]; then
  launch_background
  exit 0
fi
...
echo "EVENT_TRANSPORT_TRANSFORMER_PREFLIGHT_START"
"${PYTHON_BIN}" -m py_compile station_discrete_event_memory.py station_dataset.py \
  ...
  tools/audit_station24_event_transport_transformer.py
...
PIPELINE_ROOT="${OUTPUT_ROOT}/event_transport_transformer_${JOB_STAMP}"
TRAIN_ROOT="${PIPELINE_ROOT}/training"
RESULT_ROOT="${PIPELINE_ROOT}/validation_results"
mkdir -p "${TRAIN_ROOT}" "${RESULT_ROOT}"

echo "EVENT_TRANSPORT_TRANSFORMER_TRAINING_START source_state=raw"
"${PYTHON_BIN}" train_station24.py \
  --config configs/station24_event_transport_transformer_168h.yaml \
  --data-path "${DATA}" --output-root "${TRAIN_ROOT}" \
  --exp-name "station24_event_transport_transformer_${JOB_STAMP}" \
  --secondary-adjacency "${SECONDARY_ADJACENCY}" \
  --initialize-checkpoint "${SOURCE_CHECKPOINT}"

shopt -s nullglob
candidate_runs=("${TRAIN_ROOT}"/*_station24_event_transport_transformer_*_seed2027)
shopt -u nullglob
[[ ${#candidate_runs[@]} -eq 1 ]] || die "expected exactly one trained candidate"
CANDIDATE_RUN=${candidate_runs[0]}
FORMAL_RESULT="${RESULT_ROOT}/event_transport_transformer_raw_val_n${FORMAL_MEMBERS}_seed${GEN_SEED}"
...
"${PYTHON_BIN}" generate_station24.py \
  ...
  --result-variant geo_history_actual_event_transport_transformer_raw

COMPARISON="${PIPELINE_ROOT}/comparisons/body_tail_raw_vs_event_transport_transformer"
"${PYTHON_BIN}" tools/compare_station24_multiscale_2a.py \
  ...
  --candidate-variant geo_history_actual_event_transport_transformer_raw \
  --baseline-label "Raw Body-tail" --candidate-label "Event Transport Transformer" \
  ...
  --title "Raw Body-tail versus event transport Transformer" \
  --figure-prefix body_tail_vs_event_transport_transformer

ISOLATION_AUDIT="${PIPELINE_ROOT}/event_transport_transformer_audit"
"${PYTHON_BIN}" tools/audit_station24_event_transport_transformer.py \
  --source-run "${SOURCE_RUN}" --candidate-run "${CANDIDATE_RUN}" \
  --candidate-result "${FORMAL_RESULT}" --output-dir "${ISOLATION_AUDIT}"

TAIL="${PIPELINE_ROOT}/extreme_wind_tail/body_tail_vs_event_transport_transformer"
"${PYTHON_BIN}" tools/plot_station24_extreme_tail.py \
  ...
  --baseline-label "Raw Body-tail" --candidate-label "Event Transport Transformer"
TIMING="${PIPELINE_ROOT}/wind_event_timing/body_tail_vs_event_transport_transformer"
"${PYTHON_BIN}" tools/diagnose_station24_wind_event_timing.py \
  ...
  --candidate-variant geo_history_actual_event_transport_transformer_raw \
  --baseline-label "Raw Body-tail" --candidate-label "Event Transport Transformer"
SPECIALIZATION="${PIPELINE_ROOT}/body_tail_specialization"
...
RESULT_FILE="${LOG_FILE%.log}.results.env"
{
  ...
  echo "ALL_EVENT_TRANSPORT_TRANSFORMER_COMPLETED"
} > "${RESULT_FILE}"
cat "${RESULT_FILE}"
