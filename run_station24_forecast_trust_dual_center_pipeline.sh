...
EXPECTED_BRANCH=${EXPECTED_BRANCH:-experiment/24site-forecast-trust-dual-center}
...
launch_background() {
  ...
  log_file="${LOG_ROOT}/station24_forecast_trust_dual_center_${stamp}.log"
  pid_file="${LOG_ROOT}/station24_forecast_trust_dual_center_${stamp}.pid"
  status_file="${LOG_ROOT}/station24_forecast_trust_dual_center_${stamp}.status"
  nohup setsid env PYTHONUNBUFFERED=1 CUBLAS_WORKSPACE_CONFIG=:4096:8 \
    OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 \
    STATION24_FORECAST_TRUST_WORKER=1 JOB_STAMP="${stamp}" \
  ...
  echo "Started forecast-trust dual-center experiment"
  echo "PID: ${pid}"
  echo "Log: ${log_file}"
...
if [[ "${STATION24_FORECAST_TRUST_WORKER:-0}" != "1" ]]; then
  launch_background
  exit 0
fi
...
echo "FORECAST_TRUST_PREFLIGHT_START"
"${PYTHON_BIN}" -m py_compile station_forecast_trust.py \
  station_discrete_event_memory.py station_dataset.py train_station24.py \
  generate_station24.py src/models/station_conditioned_diffusion.py \
  tools/analyze_station24_lead_day_forecast_trust.py
"${PYTHON_BIN}" -m unittest tests.test_station24_forecast_trust \
  tests.test_station24_discrete_event_memory
...
PIPELINE_ROOT="${OUTPUT_ROOT}/forecast_trust_dual_center_${JOB_STAMP}"
TRAIN_ROOT="${PIPELINE_ROOT}/training"
RESULT_ROOT="${PIPELINE_ROOT}/validation_results"
mkdir -p "${TRAIN_ROOT}" "${RESULT_ROOT}"

echo "FORECAST_TRUST_TRAINING_START source_state=raw end_to_end=true"
"${PYTHON_BIN}" train_station24.py \
  --config configs/station24_forecast_trust_dual_center_168h.yaml \
  --data-path "${DATA}" --output-root "${TRAIN_ROOT}" \
  --exp-name "station24_forecast_trust_dual_center_${JOB_STAMP}" \
  --secondary-adjacency "${SECONDARY_ADJACENCY}" \
  --initialize-checkpoint "${SOURCE_CHECKPOINT}"

shopt -s nullglob
candidate_runs=("${TRAIN_ROOT}"/*_station24_forecast_trust_dual_center_*_seed2027)
shopt -u nullglob
[[ ${#candidate_runs[@]} -eq 1 ]] || die "expected exactly one trained candidate"
CANDIDATE_RUN=${candidate_runs[0]}
FORMAL_RESULT="${RESULT_ROOT}/forecast_trust_dual_center_raw_val_n${FORMAL_MEMBERS}_seed${GEN_SEED}"
...
"${PYTHON_BIN}" generate_station24.py \
  ...
  --result-variant geo_history_actual_forecast_trust_dual_center_event_raw

COMPARISON="${PIPELINE_ROOT}/comparisons/body_tail_raw_vs_forecast_trust_dual_center"
"${PYTHON_BIN}" tools/compare_station24_multiscale_2a.py \
  ...
  --candidate-variant geo_history_actual_forecast_trust_dual_center_event_raw \
  --baseline-label "Raw Body-tail" --candidate-label "Forecast-trust dual center" \
  ...
  --title "Raw Body-tail versus forecast-trust dual center" \
  --figure-prefix body_tail_vs_forecast_trust_dual_center
...
TAIL="${PIPELINE_ROOT}/extreme_wind_tail/body_tail_vs_forecast_trust_dual_center"
"${PYTHON_BIN}" tools/plot_station24_extreme_tail.py \
  ...
  --baseline-label "Raw Body-tail" --candidate-label "Forecast-trust dual center"
TIMING="${PIPELINE_ROOT}/wind_event_timing/body_tail_vs_forecast_trust_dual_center"
"${PYTHON_BIN}" tools/diagnose_station24_wind_event_timing.py \
  ...
  --candidate-variant geo_history_actual_forecast_trust_dual_center_event_raw \
  --baseline-label "Raw Body-tail" --candidate-label "Forecast-trust dual center"
LEAD_DAY="${PIPELINE_ROOT}/lead_day_forecast_trust"
...
RESULT_FILE="${LOG_FILE%.log}.results.env"
{
  ...
  echo "ALL_FORECAST_TRUST_DUAL_CENTER_COMPLETED"
} > "${RESULT_FILE}"
cat "${RESULT_FILE}"
